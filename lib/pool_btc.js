...
//            rpcArgs = [blockHex];
            ...
            // replace submitblock with eth_submitWork for Ethereum
            // https://github.com/ethereum/wiki/wiki/JSON-RPC#eth_submitwork
            rpcCommand = 'eth_submitWork';
            for (var i = 0; i < blockHex.length; i++)
                blockHex[i] = '0x' + blockHex[i];
        ...
        _this.daemon.cmd(rpcCommand,
            ...
            function (results) {
                for (var i = 0; i < results.length; i++) {
                    ...
                    else if (result.response === 'rejected' || result.response === false) {
...
//                job[8] = false;
            ...
            var emitShare = function () {
                _this.emit('share', isValidShare, isValidBlock, true, shareData);
            };
...
//                rpcResults[rpcCall] = r.result || r.error;
                rpcResults[rpcCall] = r.result;
                console.log(rpcCall + '  ' + r.result + '  ' + r.error);
...
//	    if (rpcResults.eth_GetWork.isvalid){
//		emitErrorLog('eth_GetWork OK ' + JSON.stringify(rpcResults[0]));
//            console.log('eth_GetWork [0]  ' + rpcResults.eth_getWork[0]);
//            console.log('eth_GetWork [1]  ' + rpcResults.eth_getWork[1]);
//            console.log('eth_GetWork [2]  ' + rpcResults.eth_getWork[2]);
//	    }
...
//                this.sendDifficulty( 0.5 );
//                this.sendDifficulty( 0.1 );
//                this.sendDifficulty( 0.05 );
//                this.sendDifficulty( 0.001 );
//                this.sendDifficulty( 0.0005 );
...
//                emitWarningLog('client.getLabel() = ' + client.getLabel() + "   params.name = " + params.name + "   params.extraNonce2 = " + params.extraNonce2 + " ntime = " + ntime + "   toDateString = " + ntime.toDateString() );
        ...
        _this.daemon.cmd('eth_getWork', [],
            function (result) {
                if (result.error) {
                    ...
                    var json_response = {
                        previousHash: result.response[0].substr(2),
                    ...
                    /*                    json_response.target = diff1.toString(16);
                     console.log(
                     "json_response.target = " + json_response.target
                     );
                     */
