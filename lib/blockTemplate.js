...
//    console.log('rpcData  ' + rpcData);
    this.target = bignum(rpcData.bits, 16);
...
//    this.prevHashReversed = util.reverseByteOrder(new Buffer(rpcData.previousHash, 'hex')).toString('hex');
    this.prevHashReversed = new Buffer(rpcData.previousHash, 'hex').toString('hex');
    ...
    this.serializeCoinbase = function (extraNonce1, extraNonce2) {
        return Buffer.concat([
            ...
            new Buffer(this.prevHashReversed, 'hex'),
            extraNonce1,
            extraNonce2
        ]);
    };
...
// Replaced
     ...
     */
//    this.diff = new Buffer(diff1, 'hex').toString(16);
