...
//var daemon = require('./daemon_stratum.js');
    ...
    function handleMessage(message) {
        switch (message.method) {
            ...
            case 'mining.extranonce.subscribe' :
                console.log( 'the client supports the mining.set_extranonce method' );
//                emitLog( 'the client supports the mining.set_extranonce method' );
//                _this.emit( 'log', 'debug', 'the client supports the mining.set_extranonce method' );
                break;
            case 'mining.submit':
...
//        SendSubmit( message.params[0], message.params[1], message.params[2] );
...
//                nonce: message.params[4]
    ...
    this.sendExtraNonce = function( nonce ) {
/*        if( difficulty === this.difficulty )
            return false;
...
var StratumServer = exports.Server = function StratumServer(options, authorizeFn) {
    ...
    this.broadcastDifficulty = function (newDiff) {
        for (var clientId in stratumClients) {
            var client = stratumClients[clientId];
